...
#define MQTTZ_MAX_MSG_SIZE              2096
        ...
        mqttz_client *dest)
{
    if (argc == 3)
    {
        ...
        // Fake origin data
        origin->data = malloc(sizeof *(origin->data) * (250 + 1));
        memset(origin->data, 'h', 250 + 1);
        origin->data[250] = '\0';
    ...
    else if (argc == 5)
    {
        ...
        int tmp_val = atoi(argv[3]);
        printf("%i\n", tmp_val);
        origin->data = malloc(sizeof *(origin->data) * (tmp_val + 1));
        memset(origin->data, 'h', tmp_val + 1);
        origin->data[tmp_val] = '\0';
        // Destination Client ID
